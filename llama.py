...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        # query: (bs, n_heads, seqlen, head_dim)
        # key: (bs, n_heads, seqlen, head_dim)
        # scores: (bs, n_heads, seqlen, seqlen)
        scores = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(head_dim)
        
        # Apply causal mask (lower triangular) to prevent attending to future tokens
        # Note: For this assignment, we don't use causal masking in attention
        # mask = torch.tril(torch.ones(seqlen, seqlen, device=query.device, dtype=torch.bool))
        ...
        # attn_weights: (bs, n_heads, seqlen, seqlen)
        # value: (bs, n_heads, seqlen, head_dim)
...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # 1) Layer normalization of the input
        # 2) Self-attention on the layer-normalized input
        # 3) Residual connection (add input to self-attention output)
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, epsilon=0.05):
        ...
        for _ in range(max_new_tokens):
            ...
            if temperature == 0.0:
                ...
                # 3) Apply mask - set probabilities below epsilon to 0
                filtered_probs = probs * mask.float()
                
                # 4) Renormalize the filtered probabilities
